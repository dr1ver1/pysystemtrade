...
def createCSVs(instr):
    ...
    results = pd.merge(
        results,
        system.positionSize.get_instrument_currency_vol(instr)
        .loc[startDate:]
        .to_frame(),
        left_index=True,
        right_index=True,
    ...
    results = pd.merge(
        results,
        system.positionSize.get_instrument_value_vol(instr).loc[startDate:].to_frame(),
        left_index=True,
        right_index=True,
        how="left",
    )
    ...
    results = pd.merge(
        results,
        system.positionSize.get_volatility_scalar(instr).loc[startDate:].to_frame(),
        left_index=True,
        right_index=True,
        how="left",
    )
    ...
    results = pd.merge(
        results,
        system.positionSize.get_subsystem_position(instr).loc[startDate:].to_frame(),
        left_index=True,
        right_index=True,
        how="left",
    )
    ...
    results = pd.merge(
        results,
        system.accounts.get_notional_position(instr).loc[startDate:].to_frame(),
        left_index=True,
        right_index=True,
        how="left",
    )
    ...
    results = pd.merge(
        results,
        system.accounts.get_actual_position(instr).loc[startDate:].to_frame(),
        left_index=True,
        right_index=True,
        how="left",
    )
    ...
    results = pd.merge(
        results,
        system.rawdata.daily_annualised_roll(instr).loc[startDate:].to_frame(),
        left_index=True,
        right_index=True,
        how="left",
    )
    ...
    results = pd.merge(
        results,
        system.positionSize.get_block_value(instr).loc[startDate:].to_frame(),
        left_index=True,
        right_index=True,
        how="left",
    )
    ...
    results = pd.merge(
        results,
        system.rawdata.raw_carry(instr).loc[startDate:].to_frame(),
        left_index=True,
        right_index=True,
        how="left",
    )
    ...
    results = pd.merge(
        results,
        system.positionSize.get_fx_rate(instr)
        .loc[startDate:]
        .resample("1B")
        .last()
        .to_frame(),
    ...
    results = pd.merge(
        results,
        system.rawdata.raw_futures_roll(instr)
        .loc[startDate:]
        .resample("1B")
        .mean()
        .to_frame(),
    ...
    results = pd.merge(
        results,
        system.rawdata.annualised_roll(instr)
        .loc[startDate:]
        .resample("1B")
        .last()
        .to_frame(),
    ...
    results = pd.merge(
        results,
        system.forecastScaleCap.get_raw_forecast(instr, "carry")
        .loc[startDate:]
        .to_frame(),
        left_index=True,
        right_index=True,
    ...
    results = pd.merge(
        results,
        system.forecastScaleCap.get_forecast_scalar(instr, "carry")
        .loc[startDate:]
        .to_frame(),
        left_index=True,
        right_index=True,
    ...
    results = pd.merge(
        results,
        system.forecastScaleCap.get_scaled_forecast(instr, "carry")
        .loc[startDate:]
        .to_frame(),
        left_index=True,
        right_index=True,
    ...
    results = pd.merge(
        results,
        system.forecastScaleCap.get_capped_forecast(instr, "carry")
        .loc[startDate:]
        .to_frame(),
        left_index=True,
        right_index=True,
    ...
    results = pd.merge(
        results,
        system.accounts.pandl_for_instrument_forecast(instr, "carry")
        .loc[startDate:]
        .to_frame(),
        left_index=True,
        right_index=True,
    ...
    results = pd.merge(
        results,
        system.forecastScaleCap.get_raw_forecast(instr, "carryOLD")
        .loc[startDate:]
        .to_frame(),
        left_index=True,
        right_index=True,
    ...
    results = pd.merge(
        results,
        system.forecastScaleCap.get_forecast_scalar(instr, "carryOLD")
        .loc[startDate:]
        .to_frame(),
        left_index=True,
        right_index=True,
    ...
    results = pd.merge(
        results,
        system.forecastScaleCap.get_scaled_forecast(instr, "carryOLD")
        .loc[startDate:]
        .to_frame(),
        left_index=True,
        right_index=True,
    ...
    results = pd.merge(
        results,
        system.forecastScaleCap.get_capped_forecast(instr, "carryOLD")
        .loc[startDate:]
        .to_frame(),
        left_index=True,
        right_index=True,
    ...
    results = pd.merge(
        results,
        system.accounts.pandl_for_instrument_forecast(instr, "carryOLD")
        .loc[startDate:]
        .to_frame(),
        left_index=True,
        right_index=True,
    ...
    results = pd.merge(
        results,
        system.rawdata.get_instrument_raw_carry_data(instr)
        .loc[startDate:]
        .resample("1B")
        .last(),
        left_index=True,
...
if __name__ == "__main__":
    ...
    if myData:
        data = csvFuturesSimData(
            csv_data_paths=dict(
                csvFuturesMultiplePricesData="private.data.futures.multiple_prices_csv",
    ...
    my_config = Config(
        "private.test_system1.config2.yaml"
    )  # How does pickling impact this?
    ...
    print(
        system.accounts.pandl_for_instrument(instr).percent.stats()
    )  ## produce % statistics
    print(
        system.accounts.pandl_for_instrument_forecast(instr, "carry").sharpe()
    )  ## Sharpe for a specific trading rule variation
    pass
